...
R-Super introduces report-supervision loss functions. The **Volume Loss** enforces the volume of the segmented tumors to match the tumor volume described in the report, and the **Ball Loss** enforces each segmented tumor to match the number, rough location, and diameter described in the report. Beyond the Volume and Ball losses, R-Super adds **report supervision for tumor attenuation** (relative brightness) and a **malignancy supervision** that learns to distinguish malignant from benign lesions using pathology reports — so R-Super not only detects tumors but identifies cancer.
